...
<div class="hero__content grid is-vertical is-vertically-centered has-no-padding is-desktop-up ml-2">
    <div class="column is-full is-flex is-vertically-centered">

        <h3>Reply to <span class="text-primary">{{ $sender->job_title }}</span></h3>
        <form action="{{ route('user-message.store') }}" method="POST" enctype="multipart/form-data">
          @csrf 
        
            <input type="text" class="form-control" name="send_to"  value="{{ $sender->job_title }}" id="inputEmail4" hidden>
